...
async function buildWithDocker(triple: string): Promise<URL> {
  ...
  const code = await run("docker", [
    ...
    "-v",
    "rdu-cargo-registry:/usr/local/cargo/registry",
    "-v",
    "rdu-cargo-git:/usr/local/cargo/git",
    "-v",
    "rdu-linux-target:/target",
    "-e",
    ...
    "sh",
    "-c",
    `cargo build --release && cp /target/release/librdu.so /work/native/prebuilt/${destName}`,
  ]);
  if (code !== 0) Deno.exit(code);
  return dest;
}
